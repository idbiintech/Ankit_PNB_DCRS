...
public class SettlmentDaoImpl extends JdbcDaoSupport implements ISettelmentDao {
  ...
  public Boolean checkfileprocessedCTC(SettlementBean settlementbeanObj) {
   String CHECK_IT = "select sum(count) from (SELECT COUNT(*) as count FROM settlement_nfs_iss_c2c_cbs whERE filedate = STR_to_date('"+settlementbeanObj.getDatepicker()+"','%Y/%m/%d')  union all SELECT COUNT(*) as count FROM settlement_nfs_iss_c2c_nfs WHERE filedate = STR_to_date('"+settlementbeanObj.getDatepicker()+"','%Y/%m/%d') ) a";
