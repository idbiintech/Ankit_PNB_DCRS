	...
	public List<ProcessDtlBean> getProcessdtls(String flag) {
		...
		try {
			...
			List<ProcessDtlBean> processDtlBeans = new ArrayList<>();
			 ...
			 String query = "select D_CAT,CBS,SWTCH, INCOMING, filename from ( SELECT a.d_cat,              CASE WHEN d_filename LIKE '%CBS%'  THEN date_format(a.filedate, '%d/%m/%y') ELSE 'NA' END cbs,              CASE WHEN d_filename LIKE '%SWITCH%' THEN date_format(a.filedate, '%d/%m/%y') ELSE 'NA' END swtch,              CASE WHEN NOT REGEXP_LIKE (d_filename, '^(CBS|SWITCH)') THEN                       date_format(a.filedate, '%d/%m/%y')                       ELSE 'NA'                  END incoming , d_filename filename               FROM (SELECT   MAX(os1.filedate) filedate,                     CASE   WHEN NOT REGEXP_LIKE (os1.file_subcategory, '^(-)')                          THEN concat(concat(os1.category,'-'),os2.file_subcategory)                       ELSE os1.category    END  d_cat,  os1.file_subcategory d_subcat, os2.filename d_filename                       FROM main_file_upload_dtls os1 INNER JOIN main_filesource os2                         ON os1.fileid = os2.fileid                           where os1." + 
