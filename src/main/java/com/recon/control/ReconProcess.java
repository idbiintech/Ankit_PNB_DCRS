...
public class ReconProcess {
  ...
  public ModelAndView ReconProcessMC(ModelAndView modelAndView, @RequestParam("category") String category, HttpServletRequest request) throws Exception {
    logger.info("***** ReconProcess.ReconProcess1 Start ****");
    List<CompareSetupBean> setupBeans = new ArrayList<>();
    String display = "";
    logger.info("RECON PROCESS GET");
  ...
  public ModelAndView VISACrossReconProcess(ModelAndView modelAndView, @RequestParam("category") String category, HttpServletRequest request) throws Exception {
    logger.info("***** ReconProcess.ReconProcess1 Start ****");
    List<CompareSetupBean> setupBeans = new ArrayList<>();
    String display = "";
    logger.info("RECON PROCESS GET");
  ...
  @ResponseBody
  public String reconRollBack(@RequestParam("filedate") String filedate, @RequestParam("subCat") String subCat, @RequestParam("Cat") String Cat) throws ParseException, Exception {
    ...
    if (!beanObj.getCategory().equalsIgnoreCase("VISACROSS")) {
      ...
      if (this.reconProcess.validateFile1(Cat, compareSetupBeans, filedate) == "Recon is already processed") {
        ...
        if (subCat.contains("MC_INT_CBS_DOM_ATM_CROSS MC_CROSS") && beanObj.getCategory().equalsIgnoreCase("MC_CROSS")) {
  ...
  @ResponseBody
  public String runProcess(@RequestParam("category") String category, FileDetailsJson dataJson, ModelAndView modelAndView, CompareSetupBean setupBean, HttpSession httpSession, @RequestParam("filedate") String filedate, @RequestParam("subCat") String subCat, @RequestParam("dollar_val") String dollar_val) {
    String msg = null;
    try {
        ...
        if (category.equalsIgnoreCase("CARDTOCARD")) {
          if (this.reconProcess.validateFile1CTC(subCat, compareSetupBeans, filedate) == null) {
            if (subCat.equalsIgnoreCase("ACQUIRER")) {
              logger.info("Inside CardtoCardRecon loop");
              if (this.reconProcess.CardtoCardACQClassify(category, filedate, compareBean.getStEntryBy())) {
          ...
          if (error_msg == null) {
            if (!category.equalsIgnoreCase("CARDTOCARD")) {
              ...
              if (category.equalsIgnoreCase("RUPAY") && subCat.equalsIgnoreCase("INTERNATIONAL")) {
                ...
                HashMap<String, Object> output = this.reconProcess.checkRupayIntRecon(filedate, category);
  ...
  @ResponseBody
  public String runVISACrossProcess(@RequestParam("category") String category, FileDetailsJson dataJson, ModelAndView modelAndView, CompareSetupBean setupBean, HttpSession httpSession, @RequestParam("filedate") String filedate, @RequestParam("subCat") String subCat, @RequestParam("dollar_val") String dollar_val) {
    String msg = null;
    try {
      ...
      if (compareSetupBeans != null) {
        ...
        if (msg == null) {
          ...
          if (error_msg == null) {
            if (!category.equalsIgnoreCase("CARDTOCARD")) {
              if (this.reconProcess.processFile(category, compareSetupBeans, filedate, Createdby, subCat)) {
                if (this.reconProcess.compareFiles(category, filedate, compareBean, subCat, dollar_val)) {
                  logger.info("RECON COMPLETED AT " + 
                      new Timestamp((new Date()).getTime()));
                  msg = "All process are completed. Please go-to settlement to see the result.";
  ...
  @ResponseBody
  public FileDetailsJson CheckStatus(@RequestParam("category") String category, FileDetailsJson dataJson, ModelAndView modelAndView, CompareSetupBean setupBean, HttpSession httpSession, @RequestParam("filedate") String filedate, @RequestParam("subcat") String subcat) {
  ...
  @ResponseBody
  public String iccwrunProcess(@RequestParam("category") String category, @RequestParam("filedate") String filedate, ModelAndView modelAndView, CompareSetupBean setupBean, HttpSession httpSession) {
