...
public class MastercardServiceImpl extends JdbcDaoSupport implements MastercardService {
	...
	public HashMap<String, Object> checkFileUpload(MastercardUploadBean beanObj, String file_name) {
		...
		try {
			...
			if (beanObj.getFileName().equalsIgnoreCase("461")) {
				qy = "SELECT COUNT(*) FROM  mastercard_461_rawdata where FILEDATE= STR_to_date('"
						+ beanObj.getFileDate() + "','%Y/%m/%d') and FILENAME = '" + file_name + "'";
			} else if (beanObj.getFileName().equalsIgnoreCase("INVOICE")) {
				qy = "SELECT COUNT(*) FROM  mastercard_invoice_rawtada where FILEDATE=  STR_to_date('"
						+ beanObj.getFileDate() + "','%Y/%m/%d')";
			} else if (beanObj.getFileName().equalsIgnoreCase("INVOICETXT")) {
				qy = "SELECT COUNT(*) FROM  mastercard_invoicetxt_rawdata where FILEDATE=  STR_to_date('"
						+ beanObj.getFileDate() + "','%Y/%m/%d') and FILENAME='" + file_name + "'";
			} else if (beanObj.getFileName().equalsIgnoreCase("INVOICEPDF")) {
				qy = "SELECT COUNT(*) FROM mastercard_invoicepdf_rawtada where FILEDATE =  STR_to_date('"
						+ beanObj.getFileDate() + "','%Y/%m/%d') and FILENAME='" + file_name + "'";
			} else if (beanObj.getFileName().equalsIgnoreCase("ATMMASTER")) {
				qy = "SELECT COUNT(*) FROM ubi_atm_master where FILEDATE=  STR_to_date('" + beanObj.getFileDate()
						+ "','%Y/%m/%d')";
			} else if (beanObj.getFileName().equalsIgnoreCase("EODTXT")) {
				qy = "SELECT COUNT(*) FROM  mastercard_eod_rawtada where FILEDATE=  STR_to_date('"
						+ beanObj.getFileDate() + "','%Y/%m/%d') and FILENAME='" + file_name + "'";
			} else {
				qy = "SELECT COUNT(*) FROM  mastercard_settlement_rawdata where FILEDATE  =  STR_to_date('"
						+ beanObj.getFileDate() + "','%Y/%m/%d')  and FILE_NAME = '" + file_name + "'";
			}
			System.out.println("qy " + qy);
			int uploadCount = ((Integer) getJdbcTemplate().queryForObject(qy, new Object[0], Integer.class)).intValue();
			if (uploadCount > 0) {
	...
	public HashMap<String, Object> checkFileUploadVISA(String file_name) {
		...
		try {
			...
			String uploadCount = (String) getJdbcTemplate().queryForObject(qy, new Object[0], String.class);
			output.put("result", Boolean.valueOf(true));
	...
	public HashMap<String, Object> checkFileUploadE057(MastercardUploadBean beanObj, String file_name) {
		...
		try {
			String qy = "";
			qy = "SELECT COUNT(*) FROM mastercard_t057_rawtada where FILEDATE=STR_to_date('" + beanObj.getFileDate()
					+ "','%Y/%m/%d') and FILENAME = '" + file_name + "'";
			System.out.println("qy " + qy);
			int uploadCount = ((Integer) getJdbcTemplate().queryForObject(qy, new Object[0], Integer.class)).intValue();
			if (uploadCount > 0) {
				output.put("result", Boolean.valueOf(false));
	...
	public HashMap<String, Object> file140Reading(MultipartFile file, MastercardUploadBean beanObj) {
		...
		try {
			if (beanObj.getFileName().equalsIgnoreCase("140")) {
				...
				ReadMastercardInvoiceRawdata readFile = new ReadMastercardInvoiceRawdata();
				output = readFile.readEODTXTFile(file, getConnection(), beanObj);
			}else if (beanObj.getFileName().equalsIgnoreCase("INVOICEPDF")) {
				ReadMastercardInvoiceRawdata readFile = new ReadMastercardInvoiceRawdata();
	...
	public HashMap<String, Object> validationRawFilesForProcessing(MastercardUploadBean beanObj) {
		...
		try {
			int checkSettProcess = ((Integer) getJdbcTemplate().queryForObject(
					"SELECT COUNT(*) FROM mastercard_settlement_all_report WHERE SETTLEMENT_DATE = TO_DATE(?,'DD/MM/YYYY') AND FILE_TYPE = ?",
					new Object[] { beanObj.getFileDate(), beanObj.getFileType() }, Integer.class)).intValue();
			if (checkSettProcess == 0) {
				int checkOverAllSettProcess = ((Integer) getJdbcTemplate().queryForObject(
						"SELECT COUNT(*) FROM mastercard_settlement_all_report where FILE_TYPE = ?",
						new Object[] { beanObj.getFileType() }, Integer.class)).intValue();
				checkOverAllSettProcess = 0;
				if (checkOverAllSettProcess > 0) {
					checkPrevDaySettProcess = ((Integer) getJdbcTemplate().queryForObject(
							"SELECT COUNT(*) FROM mastercard_settlement_all_report WHERE settlement_Date = TO_DATE(?,'DD/MM/YYYY')-1 and FILE_TYPE = ?",
							new Object[] { beanObj.getFileDate(), beanObj.getFileType() }, Integer.class)).intValue();
				} else {
				...
				if (checkPrevDaySettProcess > 0) {
					...
					int rawUploadCount = ((Integer) getJdbcTemplate().queryForObject(getRawCount,
					...
					if (rawUploadCount > 0) {
						...
						rawUploadCount = ((Integer) getJdbcTemplate().queryForObject(getRawCount,
								new Object[] { beanObj.getFileDate() }, Integer.class)).intValue();
						if (rawUploadCount > 0) {
							output.put("result", Boolean.valueOf(true));
						} else {
	...
	public HashMap<String, Object> validationForProcessing(MastercardUploadBean beanObj) {
		...
		try {
			if (beanObj.getFileType().equalsIgnoreCase("DOMESTIC")) {
				FileId140Count = ((Integer) getJdbcTemplate().queryForObject(
						"SELECT count(*) FROM  mastercard_settlement_ttum WHERE FILEDATE=STR_TO_DATE(?,'%Y/%m/%d')",
						new Object[] { beanObj.getFileDate() }, Integer.class)).intValue();
			} else {
				FileId140Count = ((Integer) getJdbcTemplate().queryForObject(
						"SELECT count(*) FROM  mastercard_int_settlement_ttum WHERE FILEDATE=STR_TO_DATE(?,'%Y/%m/%d')",
						new Object[] { beanObj.getFileDate() }, Integer.class)).intValue();
			}
	...
	public HashMap<String, Object> validationForProcessing_OLD_01112023(MastercardUploadBean beanObj) {
		...
		int FileId140Count = 0;
		try {
			int tadUploaded = ((Integer) getJdbcTemplate().queryForObject(
					"select COUNT(*) from mastercard_tad_data where filedate = to_date(?,'DD/MON/YYYY') ",
					new Object[] { beanObj.getFileDate() }, Integer.class)).intValue();
			if (tadUploaded > 0) {
				int tadFileIdCount = ((Integer) getJdbcTemplate().queryForObject(
						"select count(file_id) from (select distinct file_id from mastercard_tad_data where filedate = to_date(?,'DD/MON/YYYY') AND FILE_ID != 'TOTAL')",
						new Object[] { beanObj.getFileDate() }, Integer.class)).intValue();
				int recon_dates_count = ((Integer) getJdbcTemplate().queryForObject(
						"select count(recon_date) from (select distinct recon_date from mastercard_tad_data where settlement_date = TO_DATE(?,'DD/MM/YYYY') AND FILE_TYPE = ?  and recon_date is not null  )",
						new Object[] { beanObj.getFileDate(), beanObj.getFileType() }, Integer.class)).intValue();
				if (recon_dates_count == 2) {
					FileId140Count = ((Integer) getJdbcTemplate().queryForObject(
							"select count(file_id) from (select DISTINCT FILE_ID from mastercard_tad_data where filedate = to_date(?,'DD/MON/YYYY') AND FILE_ID != 'TOTAL' and file_id in (select file_id from mastercard_140_rawdata where settlement_date =to_date(?,'DD/MM/YYYY')))",
							new Object[] { beanObj.getFileDate(), beanObj.getFileDate() }, Integer.class)).intValue();
				} else {
					FileId140Count = ((Integer) getJdbcTemplate().queryForObject(
							"select count(file_id) from (select DISTINCT FILE_ID from mastercard_tad_data where filedate = to_date(?,'DD/MON/YYYY') AND FILE_ID != 'TOTAL' and file_id in (select file_id from mastercard_140_rawdata where settlement_date between (select TO_CHAR(to_date(min(recon_date),'DD/MM/YY')+1,'DD/MON/YYYY') from mastercard_tad_data where settlement_date = TO_DATE(?,'DD/MM/YYYY') AND FILE_TYPE = ?)  AND (SELECT TO_CHAR(TO_DATE(MAX(RECON_DATE),'DD/MM/YY'),'DD/MON/YYYY') FROM MASTERCARD_TAD_DATA where settlement_date = TO_DATE(?,'DD/MM/YYYY') AND FILE_TYPE = ?)))",

							new Object[] { beanObj.getFileDate(), beanObj.getFileDate(), beanObj.getFileType(),
									beanObj.getFileDate(), beanObj.getFileType() },
				...
				if (FileId140Count != tadFileIdCount) {
					...
					int file461Data = ((Integer) getJdbcTemplate().queryForObject(
							"SELECT count(*) FROM MASTERCARD_461_RAWDATA WHERE SETTLEMENT_DATE = ?",
							new Object[] { beanObj.getFileDate() }, Integer.class)).intValue();
					if (file461Data == 0) {
						output.put("result", Boolean.valueOf(false));
					...
					int file461DateCount = ((Integer) getJdbcTemplate().queryForObject(
							"SELECT count(settlement_date) from ( select distinct settlement_date FROM MASTERCARD_461_RAWDATA WHERE SETTLEMENT_DATE between (select TO_CHAR(to_date(min(recon_date),'DD/MM/YY')+1,'DD/MON/YYYY') from mastercard_tad_data where settlement_date = TO_DATE(?,'DD/MM/YYYY') AND FILE_TYPE = ?) AND (SELECT TO_CHAR(TO_DATE(MAX(RECON_DATE),'DD/MM/YY'),'DD/MON/YYYY') FROM MASTERCARD_TAD_DATA where settlement_date = TO_DATE(?,'DD/MM/YYYY') AND FILE_TYPE = ?) )",

							new Object[] { beanObj.getFileDate(), beanObj.getFileType(), beanObj.getFileDate(),
									beanObj.getFileType() },
							Integer.class)).intValue();
	...
	public HashMap<String, Object> validationForSettlementTTUMProcess(MastercardUploadBean beanObj) {
		HashMap<String, Object> output = new HashMap<>();
		try {
			int checkSettAlreadyProc = ((Integer) getJdbcTemplate().queryForObject(
					"select COUNT(*) from MASTERCARD_SETTLEMENT_TTUM where filedate = ? AND FILE_TYPE = ?",
					new Object[] { beanObj.getFileDate(), beanObj.getFileType() }, Integer.class)).intValue();
			if (checkSettAlreadyProc == 0) {
				int checkSettProcess = ((Integer) getJdbcTemplate().queryForObject(
						"SELECT COUNT(*) FROM MASTERCARD_SETTLEMENT_REPORT WHERE settlement_Date = TO_DATE(?,'DD/MM/YYYY') and type = ?",
						new Object[] { beanObj.getFileDate(), beanObj.getFileType() }, Integer.class)).intValue();
				if (checkSettProcess > 0) {
					output.put("result", Boolean.valueOf(true));
				} else {
