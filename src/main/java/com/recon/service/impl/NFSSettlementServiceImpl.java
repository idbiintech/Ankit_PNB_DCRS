...
public class NFSSettlementServiceImpl extends JdbcDaoSupport implements NFSSettlementService {
		...
		try {
			try {
				if (beanObj.getCategory().contains("ICD")) {
					file_id = ((Integer) getJdbcTemplate().queryForObject(
			...
			if (totalCount > 0) {
				if (beanObj.getCycle() != 0 && beanObj.getCategory().contains("NFS")) {
					...
					int dataCount = ((Integer) getJdbcTemplate().queryForObject(checkForSameDate,
							new Object[] { Integer.valueOf(file_id), beanObj.getDatepicker() }, Integer.class))
									.intValue();
					System.out.println("checkTotalCountntsl_nfs_rawdata " + checkForSameDate + "  " + dataCount);
					if (dataCount > 0) {
						validate.put("result", Boolean.valueOf(true));
	...
	public HashMap<String, Object> validatePrevFileUploadSUS(NFSSettlementBean beanObj) {
		...
		String checkTotalCount = "";
		try {
			checkTotalCount = "SELECT COUNT(*) FROM SUSPECT_RAWDATA WHERE date_format(FILEDATE,'%d%m%y') = date_format('"
					+ beanObj.getDatepicker() + "','%d%m%y')";
			totalCount = ((Integer) getJdbcTemplate().queryForObject(checkTotalCount, new Object[0], Integer.class))
					.intValue();
			...
			if (result) {
				if (beanObj.getFileName().contains("ICD")) {
					...
					file_id = ((Integer) getJdbcTemplate().queryForObject(
							...
							new Object[] { "ICD", beanObj.getCategory(), beanObj.getStSubCategory() }, Integer.class))
									.intValue();
				} else if (beanObj.getFileName().contains("NTSL-DFS")) {
					...
					file_id = ((Integer) getJdbcTemplate().queryForObject(
							...
							new Object[] { "NTSL-DFS", beanObj.getCategory(), beanObj.getStSubCategory() },
							Integer.class)).intValue();
				} else if (beanObj.getFileName().contains("NTSL-JCB")) {
					...
					file_id = ((Integer) getJdbcTemplate().queryForObject(
							...
							new Object[] { "NTSL-JCB", beanObj.getCategory(), beanObj.getStSubCategory() },
							Integer.class)).intValue();
				} else {
					...
					file_id = ((Integer) getJdbcTemplate().queryForObject(
							...
							new Object[] { "NTSL-NFS", beanObj.getCategory(), beanObj.getStSubCategory() },
							Integer.class)).intValue();
				}
				System.out.println("File id is " + file_id);
	...
	public HashMap<String, Object> iccwuploadNTSLFile(NFSSettlementBean beanObj, MultipartFile file,
			CompareSetupBean setupBean) {
		HashMap<String, Object> mapObj = new HashMap<>();
		try {
	...
	public HashMap<String, Object> ValidateDailySettProcess(NFSSettlementBean beanObj) {
		...
		try {
			...
			if (beanObj.getFileName().equalsIgnoreCase("NTSL-NFS")) {
				try {
					int adjfile_id = ((Integer) getJdbcTemplate().queryForObject(
							"SELECT FILEID FROM  main_filesource WHERE upper(FILENAME)=? AND FILE_CATEGORY = ? AND FILE_SUBCATEGORY = ?",
					...
					adjustmentFileCount = ((Integer) getJdbcTemplate().queryForObject(
							...
							new Object[] { Integer.valueOf(adjfile_id), beanObj.getDatepicker() }, Integer.class))
									.intValue();
			...
			if (uploadCount > 0 && adjustmentFileCount > 0) {
				...
				int procCount = ((Integer) getJdbcTemplate().queryForObject(alreadyProc, new Object[] {
						Integer.valueOf(file_id), beanObj.getDatepicker(), Integer.valueOf(beanObj.getCycle()) },
						Integer.class)).intValue();
				...
				if (procCount == 0) {
					...
					int firstCount = ((Integer) getJdbcTemplate().queryForObject(checkFirstTime,
							new Object[] { Integer.valueOf(file_id) }, Integer.class)).intValue();
					if (firstCount > 0) {
						int getFileCount = ((Integer) getJdbcTemplate().queryForObject(
								"SELECT FILE_COUNT FROM MAIN_FILESOURCE WHERE FILEID = ?",
								new Object[] { Integer.valueOf(file_id) }, Integer.class)).intValue();
						...
						int checkCount = ((Integer) getJdbcTemplate().queryForObject(prevdayProcess,
								new Object[] { Integer.valueOf(file_id) }, Integer.class)).intValue();
						if (checkCount > 0 && checkCount == getFileCount) {
							...
							for (String subcategory : subcategories) {
								if (beanObj.getFileName().equalsIgnoreCase("NTSL-NFS")) {
									...
									rawCount = ((Integer) getJdbcTemplate().queryForObject(checkRawData,
											new Object[] { Integer.valueOf(file_id), beanObj.getDatepicker() },
											Integer.class)).intValue();
									System.out.println("rawCount is " + rawCount);
								} else {
									file_id = ((Integer) getJdbcTemplate().queryForObject(
											"SELECT FILEID FROM  main_filesource WHERE upper(FILENAME)=? AND FILE_CATEGORY = ? AND FILE_SUBCATEGORY = ?",
											new Object[] { beanObj.getFileName().split("-")[1], beanObj.getCategory(),
													subcategory },
											Integer.class)).intValue();
									System.out.println("File id is " + file_id);
							...
							if (rawfileUploaded) {
								file_id = ((Integer) getJdbcTemplate().queryForObject(
										"SELECT FILEID FROM  main_filesource WHERE upper(FILENAME)=? AND FILE_CATEGORY = ? AND FILE_SUBCATEGORY = ?",
										new Object[] { beanObj.getFileName(), beanObj.getCategory(),
								...
								int entryCount = ((Integer) getJdbcTemplate().queryForObject(
										checkNTSLEntries, new Object[] { Integer.valueOf(file_id),
												beanObj.getDatepicker(), Integer.valueOf(beanObj.getCycle()) },
										Integer.class)).intValue();
				...
				if (checkFlag.equalsIgnoreCase("Y")) {
					...
					if (!beanObj.getFileName().contains("NFS")) {
						...
						try {
							...
							cycleCount = ((Integer) getJdbcTemplate().queryForObject(getCycleCount,
									new Object[] { Integer.valueOf(file_id) }, Integer.class)).intValue();
	...
	public HashMap<String, Object> checkMonthlyNTSLUploaded(NFSSettlementBean beanObj) {
		...
		try {
			...
			int count = ((Integer) getJdbcTemplate().queryForObject(checkEntry,
	...
	public HashMap<String, Object> ValidateForSettVoucher(NFSSettlementBean beanObj) {
		...
		try {
			...
			uploadCount = ((Integer) getJdbcTemplate().queryForObject(checkFileUploaded, new Object[] {
					Integer.valueOf(file_id), beanObj.getDatepicker(), Integer.valueOf(beanObj.getCycle()) },
					Integer.class)).intValue();
			System.out.println("Upload count " + uploadCount);
			if (uploadCount > 0) {
				...
				if (procCount > 0) {
					if (beanObj.getFileName().equalsIgnoreCase("NTSL-NFS")) {
						int adj_file_id = ((Integer) getJdbcTemplate().queryForObject(
								"SELECT FILEID FROM  main_filesource WHERE upper(FILENAME)=? AND FILE_CATEGORY = ? AND FILE_SUBCATEGORY = ?",
						...
						uploadCount = ((Integer) getJdbcTemplate().queryForObject(checkAdjFileUploaded,
								new Object[] { Integer.valueOf(adj_file_id), beanObj.getDatepicker() }, Integer.class))
										.intValue();
						...
						if (uploadCount == 0) {
							...
							int diffCount = ((Integer) getJdbcTemplate().queryForObject(diffQuery,
									new Object[] { beanObj.getDatepicker(), Integer.valueOf(beanObj.getCycle()) },
									Integer.class)).intValue();
	...
	public boolean checkSettVoucherProcess(NFSSettlementBean beanObj) {
		try {
			if (!beanObj.getFileName().contains("PBGB")) {
				...
				int procCount = ((Integer) getJdbcTemplate().queryForObject(checkProcessFlag, new Object[] {
						Integer.valueOf(file_id), beanObj.getDatepicker(), Integer.valueOf(beanObj.getCycle()) },
						Integer.class)).intValue();
				this.logger.info("Already Process count " + procCount);
				if (procCount > 0)
					if (beanObj.getFileName().equalsIgnoreCase("NTSL-NFS")) {
						String str = "DELETE FROM NFS_SETTLEMENT_VOUCHER WHERE FILEDATE ='" + beanObj.getDatepicker()
								+ "' AND CYCLE = '" + beanObj.getCycle() + "'";
						getJdbcTemplate().execute(str);
					} else if (beanObj.getFileName().equalsIgnoreCase("NTSL-DFS")) {
						String str = "DELETE FROM DFS_SETTLEMENT_VOUCHER WHERE FILEDATE ='" + beanObj.getDatepicker()
								+ "' AND CYCLE = '" + beanObj.getCycle() + "'";
						getJdbcTemplate().execute(str);
					} else {
						String str = "DELETE FROM JCB_SETTLEMENT_VOUCHER WHERE FILEDATE ='" + beanObj.getDatepicker()
								+ "' AND CYCLE = '" + beanObj.getCycle() + "'";
						getJdbcTemplate().execute(str);
					}
				return true;
	...
	public HashMap<String, Object> ValidateForAdjTTUMVISA(NFSSettlementBean beanObj) {
		HashMap<String, Object> result = new HashMap<>();
		int rawCount = 0;
		List<String> subcategories = new ArrayList<>();
		subcategories.add("ISSUER");
		...
		try {
			String checkAdjProcess = "";
			if (beanObj.getAdjType().contains("CHBK_REJECT")) {
				checkAdjProcess = "SELECT COUNT(*) FROM visa_chb_ttum WHERE NARRATION='CHBK_REJECT' AND FILEDATE=STR_TO_DATE(?,'%Y/%m/%d')";
				processCount = ((Integer) getJdbcTemplate().queryForObject(checkAdjProcess,
						new Object[] { beanObj.getDatepicker() }, Integer.class)).intValue();
			} else {

				checkAdjProcess = "SELECT COUNT(*) FROM visa_chb_ttum WHERE NARRATION='CHBK_ACPT' AND FILEDATE=STR_TO_DATE(?,'%Y/%m/%d')";
				processCount = ((Integer) getJdbcTemplate().queryForObject(checkAdjProcess,
						new Object[] { beanObj.getDatepicker(), beanObj.getAdjType() }, Integer.class)).intValue();
			}
	...
	public HashMap<String, Object> ValidateForAdjTTUMSETTL(NFSSettlementBean beanObj) {
		...
		try {
			...
			if (beanObj.getAdjCategory().contains("ICD")) {
				...
				checkAdjProcess = "SELECT COUNT(*) FROM  nfs_settlement_report WHERE filedate = str_to_date(?,'%Y/%m/%d') and cycle ='"
						+ beanObj.getCycle() + "'";
			}
			int processCount = 0;
			processCount = ((Integer) getJdbcTemplate().queryForObject(checkAdjProcess,
		...
		try {
			...
			if (processCount > 0) {
				...
				if (beanObj.getAdjType().equalsIgnoreCase("PENALTY")) {
					...
					System.out.println("checkDataPresent " + checkDataPresent);
					getcount = ((Integer) getJdbcTemplate().queryForObject(checkDataPresent, new Object[0],
							Integer.class)).intValue();
				} else if (beanObj.getAdjType().contains("CREDIT ADJUSTMENT ICCW ATM")) {
	...
	public HashMap<String, Object> ValidateForAdjTTUMTEXTSETTL(NFSSettlementBean beanObj) {
		...
		try {
			...
			processCount = ((Integer) getJdbcTemplate().queryForObject(checkAdjProcess, new Object[0], Integer.class))
					.intValue();
			if (processCount > 0) {
				result.put("result", Boolean.valueOf(false));
				result.put("msg", "Settlement TTUM is already processed.\n Please download TTUM");
	...
	public boolean checkSettlementVisa(RupayUploadBean beanObj) {
		try {
			...
			if (beanObj.getSubcategory().equalsIgnoreCase("DOMESTIC")
					&& beanObj.getSubcategory().equalsIgnoreCase("ACQUIRER")) {
				checkUpload = "SELECT COUNT(*)  FROM visa_dom_acq_settlement_report WHERE filedate = STR_to_date('"
						+ beanObj.getFileDate() + "','%Y/%m/%d') ";
			} else if (beanObj.getSubcategory().equalsIgnoreCase("DOMESTIC")
					&& beanObj.getSubcategory().equalsIgnoreCase("ISSUER")) {
				checkUpload = "SELECT COUNT(*)  FROM visa_dom_iss_settlement_report WHERE filedate = STR_to_date('"
						+ beanObj.getFileDate() + "','%Y/%m/%d') ";
			} else if (beanObj.getSubcategory().equalsIgnoreCase("INTERNATIONAL")
					&& beanObj.getSubcategory().equalsIgnoreCase("ACQUIRER")) {
				checkUpload = "SELECT COUNT(*)  FROM visa_int_acq_settlement_report WHERE filedate = STR_to_date('"
						+ beanObj.getFileDate() + "','%Y/%m/%d') ";
			} else if (beanObj.getSubcategory().equalsIgnoreCase("INTERNATIONAL")
					&& beanObj.getSubcategory().equalsIgnoreCase("ISSUER")) {
				checkUpload = "SELECT COUNT(*)  FROM visa_int_iss_settlement_report WHERE filedate = STR_to_date('"
						+ beanObj.getFileDate() + "','%Y/%m/%d') ";
			}
			procCount = ((Integer) getJdbcTemplate().queryForObject(checkUpload, new Object[0], Integer.class))
					.intValue();
			this.logger.info("Already Process count " + procCount);
			if (procCount > 0)
				return true;
	...
	public boolean checkSettlementQsparc(RupayUploadBean beanObj) {
		try {
			int procCount = 0;
			String checkProcessFlag = "SELECT COUNT(*) FROM qsparc_dscr_rawdata WHERE filedate = STR_to_date(?,'%Y/%m/%d') and cycle='"
					+ beanObj.getCycle() + "'";
			this.logger.info("checkProcessFlag sql  " + checkProcessFlag);
			procCount = ((Integer) getJdbcTemplate().queryForObject(checkProcessFlag,
					new Object[] { beanObj.getFileDate() }, Integer.class)).intValue();
		...
		try {
			...
			if (beanObj.getAdjType().equalsIgnoreCase("PENALTY")) {
				checkProcessFlag = "SELECT COUNT(*) as DATA_COUNT  FROM NFS_ADJUSTMENT_RAWDATA WHERE date_format(FILEDATE,'%d%m%y') =date_format('"
						+ beanObj.getDatepicker() + "' ,'%d%m%y') AND \r\n"
						+ "    date_format(CONVTODATE(ADJSETTLEMENTDATE),'%d%m%y')=date_format('"
						+ beanObj.getDatepicker() + "' ,'%d%m%y') AND ISR='UOB' AND TO_NUMBER(CUSTOMERPENALTY) <> 0";
				procCount = ((Integer) getJdbcTemplate().queryForObject(checkProcessFlag, new Object[0], Integer.class))
						.intValue();
			} else {
	...
	public boolean checkCoopTTUMProcess(NFSSettlementBean beanObj) {
		try {
			...
			int procCount = ((Integer) getJdbcTemplate().queryForObject(checkProcessFlag,
					new Object[] { Integer.valueOf(beanObj.getCycle()), beanObj.getDatepicker() }, Integer.class))
							.intValue();
			this.logger.info("Already Process count " + procCount);
			if (procCount > 0) {
	...
	public HashMap<String, Object> validateSettDifference(NFSSettlementBean beanObj) {
		...
		try {
			...
			int getcount = ((Integer) getJdbcTemplate().queryForObject(checkDiffTab,
					new Object[] { beanObj.getDatepicker(), Integer.valueOf(beanObj.getCycle()) }, Integer.class))
							.intValue();
			if (getcount > 0) {
				if (beanObj.getFileName().contains("NFS")) {
	...
	public List<String> getNodalData(String state) {
		...
		try {
			...
			String nodalId = (String) getJdbcTemplate().queryForObject(getNodalId, new Object[] { state },
					String.class);
			this.logger.info("Nodal Sol ID " + nodalId);
			data.add(nodalId);
			String getNodalPh = "select DISTINCT NODAL_PH from nodal_sol_master where state = ?";
			String nodalPh = (String) getJdbcTemplate().queryForObject(getNodalPh, new Object[] { state },
					String.class);
	...
	public HashMap<String, Object> CheckSettlementProcess(NFSSettlementBean beanObj) {
		...
		try {
			if (beanObj.getFileName().contains("NFS")) {
				...
				checkFlag = (String) getJdbcTemplate().queryForObject(checkSettProcess, new Object[] {
						beanObj.getDatepicker(), Integer.valueOf(beanObj.getCycle()), beanObj.getFileName() },
						String.class);
				if (checkFlag.equalsIgnoreCase("Y")) {
					result.put("result", Boolean.valueOf(true));
				} else {
				...
				checkSettProcess = "SELECT COUNT(*) from " + tableName
						+ " where filedate = date_format(?,'dd/mm/yyyy')";
	...
	public HashMap<String, Object> ValidateOtherSettProcess(NFSSettlementBean beanObj) {
		...
		try {
			...
			int rawCount = ((Integer) getJdbcTemplate().queryForObject(rawQuery,
					new Object[] { beanObj.getDatepicker() }, Integer.class)).intValue();
			if (rawCount == 0) {
				result.put("result", Boolean.valueOf(false));
