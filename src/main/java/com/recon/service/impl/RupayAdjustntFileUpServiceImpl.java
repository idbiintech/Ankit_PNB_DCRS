...
public class RupayAdjustntFileUpServiceImpl extends JdbcDaoSupport implements RupayAdjustntFileUpService {
	...
	public HashMap<String, Object> validateAdjustmentUpload(String fileDate, String cycle, String network,
			String subcategory, boolean presentmentFile, String filename) {
		...
		try {
			...
			int uploadCount = ((Integer) getJdbcTemplate().queryForObject(checkUpload,
					new Object[] { fileDate, cycle, filename }, Integer.class)).intValue();
			if (uploadCount == 0) {
	...
	public HashMap<String, Object> validatePresentmentUpload(String fileDate, String cycle, String network,
			String subcategory, boolean presentmentFile, String filename) {
		HashMap<String, Object> output = new HashMap<>();
		try {
			String tableName = "";
			...
			System.out.println("sql checkUpload " + checkUpload);
			int uploadCount = ((Integer) getJdbcTemplate().queryForObject(checkUpload,
					new Object[] { fileDate, filename }, Integer.class)).intValue();
			if (uploadCount == 0) {
				output.put("result", Boolean.valueOf(true));
			} else {
				output.put("result", Boolean.valueOf(false));
	...
	public HashMap<String, Object> rupayAdjustmentFileUpload(String fileDate, String createdBy, String cycle,
		...
		try {
			...
			String sql = "INSERT INTO " + tableName
					...
					+ " FILEDATE, CREATEDBY, CYCLE,UNQ_ID,FILENAME) " + "VALUES( ? , ? , ? ,?,?,?,?,?,"
					+ "?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?, STR_to_date(?,'%Y/%m/%d') , ?, ?,?,?)";
			...
			while ((row = csvReader1.readLine()) != null) {
				...
				} else if (subcategory.equalsIgnoreCase("DOMESTIC") && network.equalsIgnoreCase("QSPARC")
						&& line.contains("|,")) {
					ps.setString(sr_no++, "18-02-23");
				}
		...
		try {
			if (cate.equalsIgnoreCase("RUPAY")) {
				if (adjType.equalsIgnoreCase("Partial Chargeback Acceptance")) {
					...
					checkUpload = "select count(*) from rupay_network_adjustment where filedate  =str_to_date('"
							+ fileDate + "','%Y/%m/%d')  ";
				...
				if (uploadedCount > 0) {
					if (adjType.equalsIgnoreCase("REFUND")) {
						...
						uploadedCount = ((Integer) getJdbcTemplate().queryForObject(checkUpload,
								new Object[] { fileDate }, Integer.class)).intValue();
					} else {
						System.out.println("filedate passing for validation check " + fileDate);
						checkUpload = "select count(*) from rupay_adjustment_ttum where filedate  =str_to_date('"
								+ fileDate + "','%Y/%m/%d')  and TTUM_TYPE='"+adjType+"' and cate='"+cate+"'";
						uploadedCount = ((Integer) getJdbcTemplate().queryForObject(checkUpload, new Object[0],
								Integer.class)).intValue();
					}
					this.logger.info("checkUpload " + checkUpload + " " + uploadedCount);
				...
				checkUpload = "select count(*) from rupay_network_adjustment where filedate =str_to_date('" + fileDate
						+ "','%Y/%m/%d')  and FUNCTION_CODE_DESCRIPTION like '%" + adjType + "%'";
				...
				if (uploadedCount > 0) {
					if (adjType.equalsIgnoreCase("REFUND")) {
						...
						System.out.println("filedate passing for validation check " + fileDate);
						checkUpload = "select count(*) from  rupay_adjustment_ttum where filedate =str_to_date('"
								+ fileDate + "','%Y/%m/%d')  and TTUM_TYPE='Rupay_CHB' ";
						uploadedCount = ((Integer) getJdbcTemplate().queryForObject(checkUpload, new Object[0],
								Integer.class)).intValue();
					} else if (adjType.equalsIgnoreCase("Chargeback Deemed Acceptance")) {
						System.out.println("filedate passing for validation check " + fileDate);
						checkUpload = "select count(*) from   rupay_adjustment_ttum  where  filedate =str_to_date('"
								+ fileDate + "','%Y/%m/%d')  and TTUM_TYPE='Rupay_CHBD'  ";
						uploadedCount = ((Integer) getJdbcTemplate().queryForObject(checkUpload, new Object[0],
								Integer.class)).intValue();
					}
					this.logger.info("checkUpload " + checkUpload + " " + uploadedCount);
				...
				checkUpload = "select count(*) from mastercard_pos_rawdata where filedate =str_to_date('" + fileDate
						+ "','%Y/%m/%d') ";
				uploadedCount = ((Integer) getJdbcTemplate().queryForObject(checkUpload, new Object[0], Integer.class))
						.intValue();
				this.logger.info("checkUpload " + checkUpload + " " + uploadedCount);
				if (uploadedCount > 0) {
					if (adjType.equalsIgnoreCase("REFUND")) {
						checkUpload = "select count(*) from  mc_dom_refund_ttum where filedate =str_to_date('"
								+ fileDate + "','%Y/%m/%d')  and REF_TYPE='DOMESTIC'";
						uploadedCount = ((Integer) getJdbcTemplate().queryForObject(checkUpload, new Object[0],
								Integer.class)).intValue();
					} else if (adjType.equalsIgnoreCase("Chargeback Acceptance")) {
						System.out.println("filedate passing for validation check " + fileDate);
				...
				if (uploadedCount > 0) {
					if (adjType.equalsIgnoreCase("REFUND")) {
						checkUpload = "select count(*) from  mc_dom_refund_ttum where filedate =str_to_date('"
								+ fileDate + "','%Y/%m/%d')  and REF_TYPE='INTERNATIONAL'";
						uploadedCount = ((Integer) getJdbcTemplate().queryForObject(checkUpload, new Object[0],
								Integer.class)).intValue();
					} else if (adjType.equalsIgnoreCase("Chargeback Acceptance")) {
						System.out.println("filedate passing for validation check " + fileDate);
				...
				if (adjType.equalsIgnoreCase("Partial Chargeback Acceptance")) {
					String type = "RePresentment Raise";
					checkUpload = "select count(*) from rupay_qsparc_network_adjustment where filedate  = str_to_date('"
							+ fileDate + "','%Y/%m/%d')  ";
					uploadedCount = ((Integer) getJdbcTemplate().queryForObject(checkUpload, new Object[0],
							Integer.class)).intValue();
				} else {
	...
	public HashMap<String, Object> validateAdjustmentTTUMProcess(String fileDate, String adjType, String cate,
			String TTUM_TYPE) {
		HashMap<String, Object> output = new HashMap<>();
		int uploadedCount = 0;
		String checkUpload = "";
	...
	public HashMap<String, Object> validateAdjustmentTTUMProcessVISA(String fileDate, String adjType, String cate,
			String TTUM_TYPE) {
		HashMap<String, Object> output = new HashMap<>();
		int uploadedCount = 0;
		String checkUpload = "";
