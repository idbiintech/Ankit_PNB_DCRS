...
public class ReadRupayBillingReport {
						...
						for (Cell c : r) {
							...
							case Cell.CELL_TYPE_STRING:
								if (!c.getStringCellValue().trim().equalsIgnoreCase("")) {
									if (c.getStringCellValue().contains("Union Bank of India")) {
	...
	// public HashMap<String, Object> fileupload(RupayUploadBean
	// beanObj,MultipartFile file,Connection con) throws SQLException {
	// String bank_name = null, sett_bin = null,acq_iss_bin = null, in_out=null,
	// status = null, tran_cycle = null;
	// int totalcount = 0;
	// HashMap<String, Object> mapObj = new HashMap<String, Object>();
	// Workbook wb = null;
	// Sheet sheet = null;
	...
	// int final_line = 0;
	//
	// String sql = "INSERT INTO RUPAY_BILLING_RAWDATA(BANK_NAME, CR_DR,
	// SETT_BIN, txn_count, FEE_AMOUNT, FEE_AMOUNT_GST, FEE, CREATEDBY,
	// FILEDATE, CYCLE) "+
	...
	//
	// } else if
	// (file.getOriginalFilename().substring(extn).equalsIgnoreCase(".XLSX")){
	//
	// // book = new XSSFWorkbook(file.getInputStream());
	// wb = new XSSFWorkbook(file.getInputStream());
	// sheet = wb.getSheetAt(0);
	...
	// String tryDigit = bd+"";
	// int indexOfDot = tryDigit.indexOf(".");
	// int secondDigit = Integer.parseInt(tryDigit.substring(indexOfDot+1,
	// indexOfDot+2));
	// if(secondDigit > 5)
	// {
	// digit = tryDigit.substring(0,indexOfDot+3);
