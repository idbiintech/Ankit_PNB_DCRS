...
public class GenerateMCACQATMReport extends AbstractExcelView {
  protected void buildExcelDocument(Map<String, Object> map, HSSFWorkbook workbook1, HttpServletRequest request, HttpServletResponse response) throws Exception {
    ...
    SXSSFSheet sheet = workbook.createSheet("MASTERCARD_ACQ_MATCHED-ATM");
    ...
    sheet = workbook.createSheet("MASTERCARD_ACQ_UNRECON-ATM");
    ...
    sheet = workbook.createSheet("MASTERCARD_ACQ_MATCHED-CBS");
    numberStyle = workbook.createCellStyle();
    numberStyle.setDataFormat(workbook.getCreationHelper().createDataFormat().getFormat("0.00"));
    header = sheet.createRow(0);
    if (Data != null && Data.size() > 2)
    ...
    sheet = workbook.createSheet("MASTERCARD_ACQ_UNRECON-CBS");
    numberStyle = workbook.createCellStyle();
    numberStyle.setDataFormat(workbook.getCreationHelper().createDataFormat().getFormat("0.00"));
    header = sheet.createRow(0);
    if (Data != null && Data.size() > 2)
    ...
    sheet = workbook.createSheet("MASTERCARD_ACQ_KNOCKOFF");
    numberStyle = workbook.createCellStyle();
    numberStyle.setDataFormat(workbook.getCreationHelper().createDataFormat().getFormat("0.00"));
    header = sheet.createRow(0);
    if (Data != null && Data.size() > 2)
    ...
    sheet = workbook.createSheet("INT_CBS_DOM_ATM_CROSS_MATCHED");
    numberStyle = workbook.createCellStyle();
    numberStyle.setDataFormat(workbook.getCreationHelper().createDataFormat().getFormat("0.00"));
    header = sheet.createRow(0);
    if (Data != null && Data.size() > 2)
