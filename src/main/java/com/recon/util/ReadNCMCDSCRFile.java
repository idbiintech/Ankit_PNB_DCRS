...
public class ReadNCMCDSCRFile {
  private static final Logger logger = Logger.getLogger(com.recon.util.ReadNCMCDSCRFile.class);
  
  SimpleDateFormat sdf2 = new SimpleDateFormat("dd-MMM-yyyy");
  
  SimpleDateFormat sdf = new SimpleDateFormat("E MMM dd HH:mm:ss Z yyyy");
  
  public HashMap<String, Object> fileuploadINT(RupayUploadBean beanObj, MultipartFile file, Connection con) throws SQLException {
    ...
    System.out.println("fileuploadINT ");
    String query = "insert into irgcs_qsparc_int_dsr_rawdata (bank_name ,SETT_BIN , ISS_BIN , INWARD_OUTWARD , STATUS_APP ,TXNCYCLE,TXNTYPE,CHANNEL , TXN_COUNT , TXN_CCY , TXN_AMT_DR , TXN_AMT_CR , BILL_AMT_CR,BILL_AMT_DR,SET_CCY ,SET_AMT_DR , SET_AMT_CR , INT_FEE_DR , INT_FEE_CR ,MEM_INC_FEE_DR , MEM_INC_FEE_CR , OTH_FEE_AMT_DR , OTH_FEE_AMT_CR ,OTH_FEE_GST_DR , OTH_FEE_GST_CR, FINAL_SUM_CR , FINAL_SUM_DR , FINAL_NET , FILEDATE , CREATEDBY,PRODUCT_NAME,TXN_FLAG,FILENAME,CYCLE) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
    int cellIdx = 0;
    int count = 0;
            ...
            for (cellIdx = 0; cellIdx < hSSFRow.getLastCellNum(); cellIdx++) {
              ...
              try {
                ...
                if (currentCell.getCellType() == 1 && 
                  ...
                  currentCell.getStringCellValue().contains("Total") || currentCell.getStringCellValue().contains("Rupay")))
