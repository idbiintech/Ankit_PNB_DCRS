...
public class GenerateMCISSATMReport extends AbstractExcelView {
  protected void buildExcelDocument(Map<String, Object> map, HSSFWorkbook workbook1, HttpServletRequest request, HttpServletResponse response) throws Exception {
    ...
    String filename = "MCISSATM_RECON_REPORT";
    ...
    SXSSFSheet sheet = workbook.createSheet("MASTERCARD_ISS_MATCHED-2-CBS");
    ...
    sheet = workbook.createSheet("MASTERCARD_ISS_UNRECON-2-CBS");
    ...
    sheet = workbook.createSheet("MASTERCARD_ISS_UNRECON-1");
    numberStyle = workbook.createCellStyle();
    numberStyle.setDataFormat(workbook.getCreationHelper().createDataFormat().getFormat("0.00"));
    header = sheet.createRow(0);
    if (Data != null && Data.size() > 2)
    ...
    sheet = workbook.createSheet("MASTERCARD_ISS_KNOCKOFF");
    numberStyle = workbook.createCellStyle();
    numberStyle.setDataFormat(workbook.getCreationHelper().createDataFormat().getFormat("0.00"));
    header = sheet.createRow(0);
    if (Data != null && Data.size() > 2)
    ...
    sheet = workbook.createSheet("MASTERCARD_ISS_MATCHED-2");
    numberStyle = workbook.createCellStyle();
    numberStyle.setDataFormat(workbook.getCreationHelper().createDataFormat().getFormat("0.00"));
    header = sheet.createRow(0);
    if (Data != null && Data.size() > 2)
