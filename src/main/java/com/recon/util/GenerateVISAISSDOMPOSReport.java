...
public class GenerateVISAISSDOMPOSReport extends AbstractExcelView {
	...
	List<String> Excel_Headers10  = new ArrayList<String>();
	List<Object> sum_Data9 = new ArrayList<Object>();
	List<String> Excel_Headers11  = new ArrayList<String>();
	List<Object> sum_Data10 = new ArrayList<Object>();

	List<String> Excel_Headers12  = new ArrayList<String>();
	List<Object> sum_Data11 = new ArrayList<Object>();
	...
	String filename = "VISAISSDOMPOS_RECON_REPORT";	
    ...
    SXSSFSheet sheet = workbook.createSheet("ISS-POS-KNOCKOFF-CBS");  
	...
	sheet = (SXSSFSheet) workbook.createSheet("VISA-DOM-ISS-POS-MATCHED-1-CBS");
	...
	sheet = (SXSSFSheet) workbook.createSheet("VISA-DOM-ISS-POS-UNRECON-1-CBS");
	numberStyle = workbook.createCellStyle();
	numberStyle.setDataFormat(workbook.getCreationHelper().createDataFormat().getFormat("0.00"));
	// create header row
	 header = sheet.createRow(0);
...
//	
//	
	sheet = (SXSSFSheet) workbook.createSheet("VISA-DOM-ISS-POS-UNRECON-2-CBS");
	numberStyle = workbook.createCellStyle();
	numberStyle.setDataFormat(workbook.getCreationHelper().createDataFormat().getFormat("0.00"));
	// create header row
	 header = sheet.createRow(0);
...
//
	sheet = (SXSSFSheet) workbook.createSheet("VISA-DOM-ISS-POS-UNRECON-1-SWITCH");
	numberStyle = workbook.createCellStyle();
	numberStyle.setDataFormat(workbook.getCreationHelper().createDataFormat().getFormat("0.00"));
	// create header row
	 header = sheet.createRow(0);
...
//	
	sheet = (SXSSFSheet) workbook.createSheet("VISA-DOM-ISS-POS-MATCHED-2_VISA");
	numberStyle = workbook.createCellStyle();
	numberStyle.setDataFormat(workbook.getCreationHelper().createDataFormat().getFormat("0.00"));
	// create header row
	 header = sheet.createRow(0);
	...
	sheet = (SXSSFSheet) workbook.createSheet("VISA-DOM-ISS-POS-UNRECON-2_VISA");
	numberStyle = workbook.createCellStyle();
	numberStyle.setDataFormat(workbook.getCreationHelper().createDataFormat().getFormat("0.00"));
	// create header row
	 header = sheet.createRow(0);
	...
	for(int i =0 ;i < Excel_Headers7.size(); i++) {
		header.createCell(i).setCellValue(Excel_Headers7.get(i));
		header.getCell(i).setCellStyle(calculatedHeader);
	}
	//ADDING REMAINING DATA
	for(int i =0; i< sum_Data6.size() ; i++) {
		rowEntry = sheet.createRow(i+1);
		Map<String, String> map_data =  (Map<String, String>) sum_Data6.get(i);
		if(map_data.size()>0) {
	...
	sheet = (SXSSFSheet) workbook.createSheet("VISA-DOM-ISS-POS-CROSS-MATCHED-2_VISA");
	numberStyle = workbook.createCellStyle();
	numberStyle.setDataFormat(workbook.getCreationHelper().createDataFormat().getFormat("0.00"));
	// create header row
	 header = sheet.createRow(0);
	...
	for(int i =0 ;i < Excel_Headers8.size(); i++) {
		header.createCell(i).setCellValue(Excel_Headers8.get(i));
		header.getCell(i).setCellStyle(calculatedHeader);
	}
	//ADDING REMAINING DATA
	for(int i =0; i< sum_Data7.size() ; i++) {
		rowEntry = sheet.createRow(i+1);
		Map<String, String> map_data =  (Map<String, String>) sum_Data7.get(i);
		if(map_data.size()>0) {
	...
	sheet = (SXSSFSheet) workbook.createSheet("MATCHED_VISA_ORGWDL");
	numberStyle = workbook.createCellStyle();
	numberStyle.setDataFormat(workbook.getCreationHelper().createDataFormat().getFormat("0.00"));
	// create header row
	 header = sheet.createRow(0);
	...
	for(int i =0 ;i < Excel_Headers9.size(); i++) {
		header.createCell(i).setCellValue(Excel_Headers9.get(i));
		header.getCell(i).setCellStyle(calculatedHeader);
	}
	//ADDING REMAINING DATA
	for(int i =0; i< sum_Data8.size() ; i++) {
		rowEntry = sheet.createRow(i+1);
		Map<String, String> map_data =  (Map<String, String>) sum_Data8.get(i);
		if(map_data.size()>0) {
	...
	sheet = (SXSSFSheet) workbook.createSheet("UNMATCHED_VISA_ORGWDL");
	numberStyle = workbook.createCellStyle();
	numberStyle.setDataFormat(workbook.getCreationHelper().createDataFormat().getFormat("0.00"));
	// create header row
	 header = sheet.createRow(0);
	...
	for(int i =0 ;i < Excel_Headers10.size(); i++) {
		header.createCell(i).setCellValue(Excel_Headers10.get(i));
		header.getCell(i).setCellStyle(calculatedHeader);
	}
	//ADDING REMAINING DATA
	for(int i =0; i< sum_Data9.size() ; i++) {
		rowEntry = sheet.createRow(i+1);
		Map<String, String> map_data =  (Map<String, String>) sum_Data9.get(i);
		if(map_data.size()>0) {
	...
	sheet = (SXSSFSheet) workbook.createSheet("VISA-DOM-ISS-POS-MATCHED-2-CBS");
	numberStyle = workbook.createCellStyle();
	numberStyle.setDataFormat(workbook.getCreationHelper().createDataFormat().getFormat("0.00"));
	// create header row
	 header = sheet.createRow(0);
	...
	for(int i =0 ;i < Excel_Headers11.size(); i++) {
		header.createCell(i).setCellValue(Excel_Headers11.get(i));
		header.getCell(i).setCellStyle(calculatedHeader);
	}
	//ADDING REMAINING DATA
	for(int i =0; i< sum_Data10.size() ; i++) {
		rowEntry = sheet.createRow(i+1);
		Map<String, String> map_data =  (Map<String, String>) sum_Data10.get(i);
		if(map_data.size()>0) {
	...
	sheet = (SXSSFSheet) workbook.createSheet("VISA-DOM-ISS-POS-CBS-CROSS-MATCHED-2");
	numberStyle = workbook.createCellStyle();
	numberStyle.setDataFormat(workbook.getCreationHelper().createDataFormat().getFormat("0.00"));
	// create header row
	 header = sheet.createRow(0);
	...
	for(int i =0 ;i < Excel_Headers12.size(); i++) {
		header.createCell(i).setCellValue(Excel_Headers12.get(i));
		header.getCell(i).setCellStyle(calculatedHeader);
	}
	//ADDING REMAINING DATA
	for(int i =0; i< sum_Data11.size() ; i++) {
		rowEntry = sheet.createRow(i+1);
		Map<String, String> map_data =  (Map<String, String>) sum_Data11.get(i);
		if(map_data.size()>0) {
...
//	
	/*
	 * sheet = (SXSSFSheet) workbook.createSheet("ICD-ACQ-CBS-MATCHED-1");
	 * numberStyle = workbook.createCellStyle();
	 * numberStyle.setDataFormat(workbook.getCreationHelper().createDataFormat().
	 * getFormat("0.00")); // create header row header = sheet.createRow(0);
	 * if(Data!= null && Data.size() >2) { Excel_Headers7 = (List<String>)
	 * Data.get(12); } if(Data!= null && Data.size() >3) { sum_Data6 =
	 * (List<Object>) Data.get(13); } for(int i =0 ;i < Excel_Headers7.size(); i++)
	 * { header.createCell(i).setCellValue(Excel_Headers7.get(i));
	 * header.getCell(i).setCellStyle(calculatedHeader); } //ADDING REMAINING DATA
	 * for(int i =0; i< sum_Data6.size() ; i++) { rowEntry = sheet.createRow(i+1);
	 * Map<String, String> map_data = (Map<String, String>) sum_Data6.get(i);
	 * if(map_data.size()>0) { for(int m= 0 ;m < Excel_Headers7.size() ; m++) {
	 * rowEntry.createCell(m).setCellValue(map_data.get(Excel_Headers7.get(m))); } }
	 * } sheet = (SXSSFSheet) workbook.createSheet("ICD-ACQ-CBS-MATCHED-2");
	 * numberStyle = workbook.createCellStyle();
	 * numberStyle.setDataFormat(workbook.getCreationHelper().createDataFormat().
	 * getFormat("0.00")); // create header row header = sheet.createRow(0);
	 * if(Data!= null && Data.size() >2) { Excel_Headers8 = (List<String>)
	 * Data.get(14); } if(Data!= null && Data.size() >3) { sum_Data7 =
	 * (List<Object>) Data.get(15); } for(int i =0 ;i < Excel_Headers7.size(); i++)
	 * { header.createCell(i).setCellValue(Excel_Headers8.get(i));
	 * header.getCell(i).setCellStyle(calculatedHeader); } //ADDING REMAINING DATA
	 * for(int i =0; i< sum_Data7.size() ; i++) { rowEntry = sheet.createRow(i+1);
	 * Map<String, String> map_data = (Map<String, String>) sum_Data7.get(i);
	 * if(map_data.size()>0) { for(int m= 0 ;m < Excel_Headers8.size() ; m++) {
	 * rowEntry.createCell(m).setCellValue(map_data.get(Excel_Headers8.get(m))); } }
	 * }
	 * 
	 * 
	 * sheet = (SXSSFSheet) workbook.createSheet("ICD-ACQ-MATCHED-2-NPCI");
	 * numberStyle = workbook.createCellStyle();
	 * numberStyle.setDataFormat(workbook.getCreationHelper().createDataFormat().
	 * getFormat("0.00")); // create header row header = sheet.createRow(0);
	 * if(Data!= null && Data.size() >2) { Excel_Headers9 = (List<String>)
	 * Data.get(16); } if(Data!= null && Data.size() >3) { sum_Data8 =
	 * (List<Object>) Data.get(17); } for(int i =0 ;i < Excel_Headers9.size(); i++)
	 * { header.createCell(i).setCellValue(Excel_Headers9.get(i));
	 * header.getCell(i).setCellStyle(calculatedHeader); } //ADDING REMAINING DATA
	 * for(int i =0; i< sum_Data8.size() ; i++) { rowEntry = sheet.createRow(i+1);
	 * Map<String, String> map_data = (Map<String, String>) sum_Data8.get(i);
	 * if(map_data.size()>0) { for(int m= 0 ;m < Excel_Headers9.size() ; m++) {
	 * rowEntry.createCell(m).setCellValue(map_data.get(Excel_Headers9.get(m))); } }
	 * } sheet = (SXSSFSheet) workbook.createSheet("ICD-ACQ-UNRECON-2-NPCI");
	 * numberStyle = workbook.createCellStyle();
	 * numberStyle.setDataFormat(workbook.getCreationHelper().createDataFormat().
	 * getFormat("0.00")); // create header row header = sheet.createRow(0);
	 * if(Data!= null && Data.size() >2) { Excel_Headers10 = (List<String>)
	 * Data.get(18); } if(Data!= null && Data.size() >3) { sum_Data9 =
	 * (List<Object>) Data.get(19); } for(int i =0 ;i < Excel_Headers10.size(); i++)
	 * { header.createCell(i).setCellValue(Excel_Headers10.get(i));
	 * header.getCell(i).setCellStyle(calculatedHeader); } //ADDING REMAINING DATA
	 * for(int i =0; i< sum_Data9.size() ; i++) { rowEntry = sheet.createRow(i+1);
	 * Map<String, String> map_data = (Map<String, String>) sum_Data9.get(i);
	 * if(map_data.size()>0) { for(int m= 0 ;m < Excel_Headers10.size() ; m++) {
	 * rowEntry.createCell(m).setCellValue(map_data.get(Excel_Headers10.get(m))); }
	 * } }
	 */
