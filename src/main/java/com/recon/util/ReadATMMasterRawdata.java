...
public class ReadATMMasterRawdata {
  ...
  public HashMap<String, Object> read461File(MultipartFile file, Connection conn, MastercardUploadBean beanObj) {
    ...
    try {
      ...
      String REVERSE_CHARGEBACKS = "", REPRESENTMENTS = "", REPRESENTMENTS_REVERSALS = "", TOTAL = "";
      while ((line = br.readLine()) != null) {
        lineNumber++;
  ...
  public HashMap<String, Object> readATMMaster(MultipartFile file, Connection conn, MastercardUploadBean beanObj) {
    ...
    try {
      ...
      String tableName = "";
      String INSERT_QUERY = "INSERT INTO    atm_master_rawdata  (SL,ATM_ID, 1, 5, 8,  8_DIGIT_TERMINAL_ID, PROJECT, INSTALLATION_DATE, ATM_MAKE, ATM_CRM, CAPEX_OPEX, CAPEX_OPEX_ONLY, SOL_ID, BRANCH_NAME, TIER, OFFSITE_ONSITE, POPULATION_CATEGORY, ADDRESS\", DISTRICT, STATE, GEOGRAPHY_WISE_REGION, RO, ZO, RBI_ISSUED_DEPARTMENT, MSP_FINAL, CASH_VENDOR, AVG_UPTIME, AVERAGE_HITS, FILEDATE, FILENAME) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
        ...
        for (Cell c : r) {
          ...
          switch (xSSFFormulaEvaluator.evaluateInCell(c).getCellType()) {
            case 1:
              ...
              continue;
            case 0:
              digit = (new StringBuilder(String.valueOf(c.getNumericCellValue()))).toString();
              if (digit.contains("E")) {
      ...
      for (List<String> fileDatas : Data) {
        ...
        for (String col : fileDatas) {
          ...
          ps.setString(30, file.getOriginalFilename());
