...
public class ReadNFSAdjustmentFile {
  private static final Logger logger = Logger.getLogger(com.recon.util.ReadNFSAdjustmentFile.class);
  ...
  public HashMap<String, Object> fileupload(NFSSettlementBean beanObj, MultipartFile file, Connection con) throws SQLException {
    ...
    System.out.println("tableName " + tableName + "file name " + beanObj.getFileName() + " " + file.getOriginalFilename());
    if (beanObj.getCreatedBy().equalsIgnoreCase("ICD")) {
      ...
      sql = "INSERT INTO dfs_adjustment_rawdata (Txnuid,TxnType,u_id,Adjdate,Adjtype,ACQ,ISR,Response,TxnDate,TxnTime,RRN,ATMID,CardNo,ChbDate,ChbRef,TxnAmount,AdjAmount,ACQFee,ISSFee,ISSFeeSW,NpciFee,AcqFeeTax,IssFeeTax,NpciTAX,AdjRef,BankAdjRef,AdjProof,ReasonDesc,Pincode,ATMLocation,MultiDisputeGroup,FCQM,AdjSettlementDate,CustomerPenalty,AdjTime,Cycle,TAT_Expiry_Date,AcqSTLAmount,AcqCC,Pan_Entry_Mode,Service_Code,Card_Dat_Input_Capability,MCC_Code ,CREATEDBY,FILEDATE,FILENAME) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
        ...
        sql = "INSERT INTO jcb_adjustment_rawdata (Txnuid,TxnType,u_id,Adjdate,Adjtype,ACQ,ISR,Response,TxnDate,TxnTime,RRN,ATMID,CardNo,ChbDate,ChbRef,TxnAmount,AdjAmount,ACQFee,ISSFee,ISSFeeSW,NpciFee,AcqFeeTax,IssFeeTax,NpciTAX,AdjRef,BankAdjRef,AdjProof,ReasonDesc,Pincode,ATMLocation,MultiDisputeGroup,FCQM,AdjSettlementDate,CustomerPenalty,AdjTime,Cycle,TAT_Expiry_Date,AcqSTLAmount,AcqCC,Pan_Entry_Mode,Service_Code,Card_Dat_Input_Capability,MCC_Code ,CREATEDBY,FILEDATE,FILENAME) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
      } else {
    ...
    try {
      ...
      List<Integer> DateCol = new ArrayList<>();
      DateCol.add(Integer.valueOf(4));
      DateCol.add(Integer.valueOf(9));
      DateCol.add(Integer.valueOf(14));
      DateCol.add(Integer.valueOf(33));
      for (Row r : sheet) {
        if (r.getRowNum() > 0) {
          int cellCount = 1;
          for (Cell c : r) {
