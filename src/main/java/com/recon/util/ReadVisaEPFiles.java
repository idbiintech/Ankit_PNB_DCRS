	...
	public void fileupload(VisaUploadBean beanObj, MultipartFile file, Connection conn) throws SQLException {
		...
		try {
			...
			String REPORT_ID = "", REPORT_DESCRIPTION = "", PAGE = "", REPORTING_FOR = "",
					...
					SETTLEMENT_CURRENCY = "", DESCRIPTION = "",DESCRIPTION2="",DESCRIPTION3="", SUB_DESCRIPTION = "", TOTAL_ACQUIRER = "",
					TOTAL_ISSUER = "", TOTAL_TYPE = "", COUNT = "", CREDIT_AMOUNT = "", DEBIT_AMOUNT = "",
					TOTAL_AMOUNT = "", CLEARING_CURRENCY = "", TRANSACTIONS_TYPE = "", CLEARING_AMOUNT = "",
					INTERCHANGE_CREDIT_AMOUNT = "", INTERCHANGE_DEBIT_AMOUNT = "", INTERCHANGE_AMOUNT = "",
					REIMBURSEMENT_CREDIT_AMOUNT = "", REIMBURSEMENT_DEBIT_AMOUNT = "", BUSINESS_MODE = "",
					TOTAL_CLEARING_CURRENCY = "", TOTAL_CLEARING_AMOUNT = "", TOTAL_COUNT = "", VISA_CHARGE_CREDIT = "",
					VISA_CHARGE_DEBIT = "",OPT_ISS_FEE="",CONVERSION_FEE="",INTERCHANGE_AMOUNT2="",CONVERSION_FEE2="",TABLE_ID="";
			...
			while ((line = br.readLine()) != null) {
				...
				if (line.contains(":")) {
					...
					if (VSS.contains("VSS-110") || VSS.contains("VSS-110-M") || VSS.contains("VSS-120")
							...
							|| VSS.contains("VSS-140") || VSS.contains("VSS-140-M") ||  VSS.contains("VSS-210") || VSS.contains("VSS-210-M")) {
						...
						if (values[0].trim().equalsIgnoreCase("REPORT ID")) {
							...
							if (REPORT_ID.contains("VSS-110-M") || REPORT_ID.contains("VSS-120-M")
									|| REPORT_ID.contains("VSS-130-M") || REPORT_ID.contains("VSS-140-M")  || REPORT_ID.contains("VSS-210-M")) {
...
//
				...
				if (VSS.contains("VSS-110") || VSS.contains("VSS-110-M")) {
					...
					if (line.trim().contains("ISSUER TRANSACTIONS") || line.trim().contains("ACQUIRER TRANSACTIONS") || line.trim().contains("OTHER TRANSACTIONS")  ) {

						TRANSACTIONS_TYPE = line.trim();
					} else if (line.trim().startsWith("PURCHASE") || line.trim().startsWith("QUASI-CASH")
							|| line.trim().startsWith("MERCHANDISE CREDIT") || line.trim().startsWith("ATM CASH") ) {

						DESCRIPTION = line.trim();

					}

					if (line.trim().contains("ORIGINAL SALE") || line.trim().contains("ORIGINAL SALE        RVRSL")
							|| line.trim().contains("DISPUTE FIN") || line.trim().contains("DISPUTE RESP FIN")
					...
					if (line.trim().contains("ISSUER TRANSACTIONS") || line.trim().contains("ACQUIRER TRANSACTIONS")) {
						...
						TRANSACTIONS_TYPE = line.trim();
							...
							|| line.trim().startsWith("QUASI-CASH CREDIT") || line.trim().startsWith("ATM CASH") 	|| line.trim().startsWith("ATM DECLINE") 	|| line.trim().startsWith("ATM BALANCE INQUIRY") ) {
				...
				if ( 			line.trim().startsWith("VISA INTERNATIONAL")
						|| line.trim().startsWith("VISA A.P.")
						) {
					...
					if (line.trim().contains("CHNINT PRGM AP") ||line.trim().contains("NEPAL - INDIA") ||line.trim().contains("TUR SPRPREM CARD") || line.trim().contains("AP CLS") || line.trim().contains("AP GLD")|| line.trim().contains("AP ALT")
							...
							|| line.trim().contains("ATM AF")
							|| line.trim().contains("ATM DCC AF") || line.trim().startsWith("TUR SPRPRM DGD")
							|| line.trim().contains("AP ALT GLD")
							|| line.trim().contains("ATM DCC AF") || line.trim().contains("AP ATM BAL INQ")
							|| line.trim().contains("AP SIG") || line.trim().contains("AP CLS")
							|| line.trim().contains("AP ALT CLS") || line.trim().contains("ATM DECLINE")
							|| line.trim().contains("AP ATM BAL INQ") || line.trim().contains("AP GLD")
							|| line.trim().contains("AP ALT PLT") || line.trim().contains("AP ALT SIG")
							|| line.trim().contains("AP PLT") || line.trim().contains("AP ALT GLD")
							...
							|| line.trim().contains("COMCL-BUS") || line.trim().startsWith("CREDIT ADJUSTMENT") || line.trim().startsWith("TOTAL ORIGINAL SALE RVRSL") || line.trim().startsWith("AP ORIG CREDIT") || line.trim().startsWith("TUR NONPREM ALT")
					) {
						if (DESCRIPTION.contains("PURCHASE") || DESCRIPTION.contains("ATM CASH")
								|| DESCRIPTION.contains("QUASI-CASH") || DESCRIPTION.contains("MERCHANDISE CREDIT")
								...
								|| DESCRIPTION.contains("DISPUTE RESP FIN       RVRSL") || DESCRIPTION.contains("ATM DECLINE") || DESCRIPTION.contains("ATM DECLINE")) {
					...
					if (line.trim().contains("NEPAL - INDIA") || line.trim().contains("AP CLS") || line.trim().contains("AP GLD")|| line.trim().contains("AP ALT") || line.trim().startsWith("TUR SPRPRM DGD")
							...
							|| line.trim().contains("SPR PREMIUM CARD")		|| line.trim().contains("ATM TRAVEL") || line.trim().contains("AP ATM TRAVEL")|| line.trim().contains("ATM DCC AF")
							|| line.trim().contains("NONPREMIUM ALT") 
							|| line.trim().contains("PREMIUM DGD") 
							|| line.trim().contains("TUR NONPREM CARD") || line.trim().contains("TUR NONPREM DGD")
							|| line.trim().contains("ORIGINAL CREDIT") || line.trim().contains("TUR PREM DGD")
							|| line.trim().contains("AP STD PLT") || line.trim().contains("BUS DGD")|| line.trim().contains("AP STD") || line.trim().contains("TUR PREM CARD")
							|| line.trim().contains("COMCL-BUS") || line.trim().startsWith("CREDIT ADJUSTMENT") || line.trim().startsWith("TOTAL ORIGINAL SALE RVRSL") || line.trim().startsWith("AP ORIG CREDIT") || line.trim().startsWith("TUR NONPREM ALT")
						...
						if (DESCRIPTION.contains("PURCHASE") || DESCRIPTION.contains("ATM CASH")
								...
								|| DESCRIPTION.contains("DISPUTE FIN") || DESCRIPTION.contains("DISPUTE RESP FIN")
								|| DESCRIPTION.contains("ATM BALANCE INQUIRY") || DESCRIPTION.contains("DISPUTE FIN")
								|| DESCRIPTION2.contains("DISPUTE RESP FIN       RVRSL") || DESCRIPTION2.contains("PURCHASE") || DESCRIPTION2.contains("ATM CASH")
								|| DESCRIPTION2.contains("QUASI-CASH") || DESCRIPTION2.contains("MERCHANDISE CREDIT")
								|| DESCRIPTION2.contains("QUASI-CASH CREDIT") || DESCRIPTION2.contains("ORIGINAL CREDIT")
								|| DESCRIPTION2.contains("ORIGINAL") || DESCRIPTION2.contains("ORIGINAL SALE")
					...
					} else if (line.trim().startsWith("ORIGINAL CREDIT") || line.trim().startsWith("ORIGINAL") || line.trim().startsWith("CREDIT ADJUSTMENT")
							|| line.trim().startsWith("ORIGINAL SALE")
							|| line.trim().startsWith("ORIGINAL SALE          RVRSL")
							|| line.trim().startsWith("ORIGINAL WITHDRAWAL    RVRSL")
							|| line.trim().startsWith("DISPUTE FIN") || line.trim().startsWith("DISPUTE RESP FIN")
					...
					if (line.trim().contains("VISA INTERNATIONAL") || line.trim().contains("TOTAL CURRENCY CONVERSION FEES")
					...
					if (line.trim().startsWith("ATM CASH") || line.trim().startsWith("POS AUTHORIZATION") || line.trim().startsWith("POS AUTH REVERSAL")
							|| line.trim().startsWith("POS DECLINE") || line.trim().startsWith("ATM BALANCE INQUIRY")
							|| line.trim().startsWith("ATM DECLINE")
						...
						DESCRIPTION = line.trim();
							...
							|| line.trim().startsWith("NON-FINANCIAL TRANSACTIONS")|| line.trim().startsWith("NON-FINANCIAL TRANSACTIONS")) {
			/*		if(DESCRIPTION.contains("FINANCIAL TRANSACTIONS") && DESCRIPTION.contains("NON-FINANCIAL TRANSACTIONS")) {
					...
					if (line.trim().contains("RECEIVED FROM VISA") || line.trim().contains("TOTAL NON-FINANCIAL")
						...
						if (line.trim().startsWith("ATM CASH") || DESCRIPTION.startsWith("POS AUTHORIZATION") || DESCRIPTION.startsWith("POS AUTH REVERSAL")
								|| DESCRIPTION.startsWith("POS DECLINE")
						...
						//	System.out.println("ORIGINAL SALE " + line.trim());
							...
							PS900.addBatch();
							DESCRIPTION = DESCRIPTION.replaceAll("FINANCIAL TRANSACTIONS","").replaceAll("NON-FINANCIAL TRANSACTIONS", "");
							SUB_DESCRIPTION = "";
							COUNT = "";
							CREDIT_AMOUNT = "";
							DEBIT_AMOUNT = "";
