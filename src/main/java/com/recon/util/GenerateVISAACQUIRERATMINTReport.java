...
public class GenerateVISAACQUIRERATMINTReport extends AbstractExcelView {
	protected void buildExcelDocument(Map<String, Object> map, HSSFWorkbook workbook1, HttpServletRequest request, HttpServletResponse response) throws Exception {
	    ...
	    String filename = (name != null && !name.equalsIgnoreCase("")) ? name : "VISA_SETTLEMENT_REPORT";
	    response.setContentType("application/vnd.ms-excel");
	    response.setHeader("Content-disposition", "attachment; filename=" + filename + ".xlsx");
	    ServletOutputStream servletOutputStream = response.getOutputStream();
	    SXSSFWorkbook workbook = new SXSSFWorkbook();
	    int maxRowCount = 1000000;
	    createSheet(workbook, "SWITCH_KNOCKOFF", Data, 0, 1, maxRowCount);
	    createSheet(workbook, "VISA-ACQ-INT-ATM-UNRECON-1-SWITCH", Data, 2, 3, maxRowCount);
	    createSheet(workbook, "VISA-ACQ-INT-ATM-MATCHED-1-CBS", Data, 4, 5, maxRowCount);
	    createSheet(workbook, "VISA-ACQ-INT-ATM-UNRECON-1-CBS", Data, 6, 7, maxRowCount);
	    createSheet(workbook, "ACQ-CBS-KNOCKOFF", Data, 8, 9, maxRowCount);
	    createSheet(workbook, "VISA-ACQ-INT-ATM-MATCHED-2-CBS", Data, 10, 11, maxRowCount);
	    createSheet(workbook, "VISA-ACQ-INT-ATM-UNRECON-2-CBS", Data, 12, 13, maxRowCount);
	    createSheet(workbook, "VISA-ACQ-INT-ATM-MATCHED-2-VISA", Data, 14, 15, 
	        maxRowCount);
	    createSheet(workbook, "VISA-ACQ-INT-ATM-UNRECON-2-VISA", Data, 16, 17, maxRowCount);
	    createSheet(workbook, "ORG-WDL-REVERSAL-MATCHED-VISA", Data, 18, 19, maxRowCount);
