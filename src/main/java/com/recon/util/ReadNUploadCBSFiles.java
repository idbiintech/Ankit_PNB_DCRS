...
public class ReadNUploadCBSFiles {
	...
	public HashMap<String, Object> uploadUbiCBSData(CompareSetupBean setupBean, Connection con, MultipartFile file,
		...
		String InsertQuery9976 = "INSERT INTO   cbs_mastercard_rawdata (tran_particular1,seq_num,card_no,tran_amount,value_date,rcre_time,sol_id,tran_date,part_tran_type,atm_gl,tran_remarks,cummulative_balance,tran_particular,narration ,tran_id,uid,tran_remarks1,posted_time,verified_time,approval_code,FILEDATE,FILENAME,CREATEDDATE)          VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
		...
		try {
			...
			if (setupBean.getP_FILE_NAME().contains("5165005139948")
					...
					|| setupBean.getP_FILE_NAME().contains("5165005712453")) {
				int linecount = 0;
				while ((thisline = br.readLine()) != null) {
					linecount++;
			...
			if (setupBean.getP_FILE_NAME().contains("5165005139949")
					|| setupBean.getP_FILE_NAME().contains("5165005811351")
					|| setupBean.getP_FILE_NAME().contains("5165005811352")
					|| setupBean.getP_FILE_NAME().contains("5165005712451")) {
				while ((thisline = br.readLine()) != null) {
					if (!thisline.toUpperCase().trim().contains("HDRCBS") && !thisline.trim().contains("FTRCBS")
			...
			PreparedStatement ps9976 = con.prepareStatement(InsertQuery9976);
			if (setupBean.getP_FILE_NAME().contains("5165005139976")
					|| setupBean.getP_FILE_NAME().contains("5165005139977")
					|| setupBean.getP_FILE_NAME().contains("5165005139979")
					|| setupBean.getP_FILE_NAME().contains("5165005139980")
					|| setupBean.getP_FILE_NAME().contains("5165005139981")) {
				...
				while ((thisline = br.readLine()) != null) {
					if (!thisline.toUpperCase().trim().contains("HDRCBS") && !thisline.trim().contains("FTRCBS")) {
						...
						String[] splitData = thisline.split("\\|", -1);
						for (int i = 1; i <= splitData.length; i++)
							ps9976.setString(sr_no++, splitData[i - 1]);
						ps9976.setString(21, setupBean.getFileDate());
						ps9976.setString(22, setupBean.getP_FILE_NAME());
						ps9976.setString(23, setupBean.getFileDate());
						ps9976.addBatch();
					}
					if (++batchCount % batchSize == 0) {
						batchNumber++;
			...
			if (setupBean.getP_FILE_NAME().contains("516505139970")) {
				while ((thisline = br.readLine()) != null) {
					if (!thisline.toUpperCase().trim().contains("HDRCBS") && !thisline.trim().contains("FTRCBS")
							&& !thisline.trim().contains("solid") && !thisline.trim().contains("FTRCBS")
							&& !thisline.trim().contains("------") && thisline.trim().length() != 46
	...
	public HashMap<String, Object> uploadUbiSWITCHData(CompareSetupBean setupBean, Connection con, MultipartFile file,
			FileSourceBean sourceBean) {
		HashMap<String, Object> output = new HashMap<>();
		String thisline = null;
		int lineNumber = 0, feesize = 1;
