...
function DownloadAdjTTUM() {
		...
		$.ajax({
			...
			type:'POST',
			url :'ValidateDownloadVisaAdjTTUM.do',
			async: true,
			beforeSend : function() {
				showLoader();
			},
			
			data:{datepicker:datepicker,fileName:filename,adjType:adjType},
...
function processAdjTTUM() {
	...
	$.ajax({
		...
		type:'POST',
		//url :'NFSAdjustmentProcess.do', VisaAdjustmentTTUM1
		url :'VisaAdjustmentTTUM1.do',
		async: true,
		beforeSend : function() {
			showLoader();
		},
		
		data:{datepicker:datepicker,fileName:filename,category:category,adjType:adjType},
		success:function(response){
			alert(response);
