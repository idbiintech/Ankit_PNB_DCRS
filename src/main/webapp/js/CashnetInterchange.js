...
function processInterchange() {
		...
		$.ajax({
			...
			type:'POST',
			url :'ProcessCashnetInterchange.do',
			async: true,
			beforeSend : function() {
				showLoader();
			},
			
			data:{category:category,datepicker:datepicker,stSubCategory:stSubCategory,timePeriod:timePeriod},
...
function DownloadInterchange() {
	...
	if(ValidateData())  {
		$.ajax({
			...
			type:'POST',
			url :'ValidateDownloadInterchangeReport.do',
			async: true,
			beforeSend : function() {
				showLoader();
			},
